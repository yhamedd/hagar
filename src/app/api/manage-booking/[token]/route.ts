...
export async function PUT(request: Request, context: Context) {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (!isDayAvailable(row.technician, day) || !generateTimeSlots(row.technician).includes(bookingTime) || !appointmentFitsSchedule(row.technician, bookingTime, duration)) throw new ManageError("Time is outside the technician's schedule", 400);
